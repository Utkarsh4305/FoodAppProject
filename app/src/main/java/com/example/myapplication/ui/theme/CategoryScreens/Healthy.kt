...
@Composable
fun HealthyScreen(navController: NavHostController) {
    ...
    LaunchedEffect(Unit) {
        coroutineScope.launch {
            products = fetchProductsFromFirestoreHealthy()
            isLoading = false
    ...
    Column(modifier = Modifier
        ...
        TextField(
            ...
            placeholder = { Text(text = "Search Healthy Items") },
...
suspend fun fetchProductsFromFirestoreHealthy(): List<Uploading> {
    val db = FirebaseFirestore.getInstance()
    return try {
        val snapshot: QuerySnapshot = db.collection("products")
            .whereEqualTo("category", "Healthy")
            .get()
            .await()
...
@Composable
fun HealthyScreenPreview() {
    HealthyScreen(navController = rememberNavController())
}
