...
@Composable
fun PizzaScreen(navController: NavHostController) {
    ...
    LaunchedEffect(Unit) {
        coroutineScope.launch {
            products = fetchProductsFromFirestorePizza()
            isLoading = false
    ...
    Column(modifier = Modifier
        ...
        TextField(
            ...
            placeholder = { Text(text = "Search Pizzas") },
...
suspend fun fetchProductsFromFirestorePizza(): List<Uploading> {
    val db = FirebaseFirestore.getInstance()
    return try {
        val snapshot: QuerySnapshot = db.collection("products")
            .whereEqualTo("category", "Pizza")
            .get()
            .await()
...
fun PizzaScreenPreview() {
    PizzaScreen(navController = rememberNavController())
}
